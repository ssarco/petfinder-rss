<?php function shelter_info($sid, $public_key) { $sinfo=simplexml_load_file( "http://api.petfinder.com/shelter.get?key=" . $public_key . "&id=" . $sid . ""); foreach ($sinfo->
	shelter as $shelter_xml) { $shid = $shelter_xml->id; $sname = $shelter_xml->name; $sname = htmlspecialchars($sname); $sname = strip_tags($sname, "
	<a><b><i>");
	...
	*/
	...
	$rss .= '  <channel>' . "\n";
	$rss .= '    <atom:link href="' . ($channel->atomLinkHref ? $channel->
	atomLinkHref : $selfUrl) . '" rel="self" type="application/rss+xml" />' . "\n";
	$rss .= '    <title>' . $channel->title . '</title>' . "\n";
	$rss .= '    <link>' . $channel->link . '</link>' . "\n";
	$rss .= '    <description>Data by: Petfinder.com and hosted by Anthony Aldridge ' .
	" // Agency Listing Information is: " . $channel->description . '. FYI: If your listing is no longer aggregating, use: https://atechconsultant.com/rss.old.php</description>' . "\n";
	...
	if (!empty($channel->cloud_domain) || !empty($channel->cloud_path) || !empty($channel->
	cloud_registerProcedure) || !empty($channel->cloud_protocol)) {
